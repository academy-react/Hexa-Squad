...
const NewsCard = ({ name, img, description, views, date, id }) => {
  return (
    ...
    >
      <div className="w-[440px] h-[220px]  md:h-[240px]  md:w-[600px] cursor-pointer rounded-xl ">
          ...
          <div className="absolute cursor-pointer group mr-[220px] md:mr-[260px] lg:mr-[265px] mt-5 md:mt-6 lg:mt-7 ">
            ...
            <div className="flex flex-row my-2">
              ...
              <h2 className="news-date text-indigo-900 text-xs dark:text-indigo-200 mt-[3px] mx-2">
                <i
                  className="bi bi-calendar-event text-sm dark:text-indigo-200 ml-1 "
                  dir="ltr"
                ></i>
                {GregorianToSolar(date)}
              </h2>
            </div>
            <h2 className="news-description  md:text-base h-[60px] w-[95%] text-justify text-sm md:my-2 mt-2 pl-6 text-lightblue dark:text-[#cbc7ff] dark:opacity-90 dark:group-hover:text-[#DEDCFF] opacity-60">
              {description}
            </h2>
        ...
        <img
          src={img == null || undefined ? article : img}
